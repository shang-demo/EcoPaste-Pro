<a href="https://github.com/3899/EcoPaste-Pro">
  <img src="https://socialify.git.ci/3899/EcoPaste-Pro/image?description=1&descriptionEditable=%E9%81%A9%E7%94%A8%E6%96%BC%20Windows%E3%80%81MacOS%20%E5%92%8C%20Linux(x11)%20%E7%9A%84%E9%96%8B%E6%BA%90%E5%89%AA%E8%B2%BC%E6%9D%BF%E7%AE%A1%E7%90%86%E5%B7%A5%E5%85%B7%E3%80%82&font=Source%20Code%20Pro&logo=https%3A%2F%2Fgithub.com%2FEcoPasteHub%2FEcoPaste%2Fblob%2Fmaster%2Fpublic%2Flogo.png%3Fraw%3Dtrue&name=1&owner=1&pattern=Floating%20Cogs&theme=Auto" alt="EcoPaste" />
</a>

<div align="center">
  <br/>

  <div>
    繁體中文 | <a href="./README.md">简体中文</a> | <a href="./README.en-US.md">English</a> | <a href="./README.ja-JP.md">日本語</a>
  </div>
  
  <br/>

  <div>
    <a href="https://github.com/3899/EcoPaste-Pro/releases">
      <img
        alt="Windows"
        src="https://img.shields.io/badge/-Windows-blue?style=flat-square&logo=data:image/svg+xml;base64,"
      />
    </a >  
    <a href="https://github.com/3899/EcoPaste-Pro/releases">
      <img
        alt="MacOS"
        src="https://img.shields.io/badge/-MacOS-black?style=flat-square&logo=apple&logoColor=white"
      />
    </a >
    <a href="https://github.com/3899/EcoPaste-Pro/releases">
      <img 
        alt="Linux"
        src="https://img.shields.io/badge/-Linux-yellow?style=flat-square&logo=linux&logoColor=white" 
      />
    </a>
  </div>

  <div>
    <a href="./LICENSE">
      <img
        src="https://img.shields.io/github/license/3899/EcoPaste-Pro?style=flat-square"
      />
    </a >
    <a href="https://github.com/3899/EcoPaste-Pro/releases">
      <img
        src="https://img.shields.io/github/package-json/v/3899/EcoPaste-Pro?style=flat-square"
      />
    </a >
    <a href="https://github.com/3899/EcoPaste-Pro/releases">
      <img
        src="https://img.shields.io/github/downloads/3899/EcoPaste-Pro/total?style=flat-square"
      />  
    </a >
  </div>

  <br/>

  <picture>
    <img src="./static/Clipboard.png" width="100%" alt="Clipboard" />
    <br/><br/>
    <img src="./static/Settings - Clipboard.png" width="100%" alt="Settings Clipboard" />
    <br/><br/>
    <img src="./static/Settings - Storage Statistics.png" width="100%" alt="Storage Statistics" />
    <br/><br/>
    <img src="./static/Settings - Shortcuts.png" width="100%" alt="Shortcuts" />
    <br/><br/>
    <img src="./static/Settings - Data Backup.png" width="100%" alt="Data Backup" />
    <br/><br/>
    <img src="./static/Settings - Data Sync.png" width="100%" alt="Data Sync" />
  </picture>
</div>

## 🌟 分支說明

> 本倉庫是 [EcoPasteHub/EcoPaste](https://github.com/EcoPasteHub/EcoPaste) 的 Fork 版本，在官方原版基礎上，重構並新增了跨設備資料同步、WebDAV 雲端資料資產安全備份、全景儲存統計等多項高級功能，同時全面增強了剪貼簿內容的智慧分類識別引擎與日常互動的沉浸式體驗。

## 📥 下載連結

### 🔗 官方原版
- 🌐 GitHub 主頁：[EcoPasteHub/EcoPaste](https://github.com/EcoPasteHub/EcoPaste)
- 📥 官方下載：[Releases 頁面](https://github.com/EcoPasteHub/EcoPaste/releases)
- 📚 使用文件：[EcoPaste 官網](https://ecopaste.cn/)

### 🔗 本 Fork 分支
- 📥 最新版本: [前往 Releases 下載最新構建程式包](https://github.com/3899/EcoPaste-Pro/releases/latest)
- 📚 移動端資料同步指南
  - [Android 同步配置指南](docs/Android/README.md)（以 Tasker 為例提供完整配套文件，使用 MacroDroid 等自動化軟體的用戶完全可參考其邏輯舉一反三自行編寫）
  - iPhone 同步配置指南（整理中，待補充...）

## 📖 功能介紹

- 🎉 基於 Tauri v2 開發，輕量高效，跨平臺體驗更進一步。
- 💻 支持 Windows、macOS 和 Linux（x11），在多設備間無縫切換。
- ✨ 簡潔直觀的使用者介面，操作簡單，零門檻上手，開箱即用。
- 📋 支持純文字、富文字、HTML、圖片和檔案類型的剪貼板內容。
- 🔒 數據本地存儲，確保用戶隱私安全，數據完全掌控在用戶手中。
- 📝 支持備註功能，輕鬆分類、管理和檢索，讓工作更高效。
- ⚙️ 豐富的個性化設定，滿足不同用戶需求，打造專屬體驗。
- 🤝 完善的檔案與社區支持，與開發者共同探索與成長。
- 🧩 持續優化中，更多驚喜功能等你發現。



### Pro.5.x <font size="-2" color="gray">（基於原版v0.6.0-beta.3分支）</font>

#### Pro.5.6

##### ✨ 新增功能
- **🖱️ 滑鼠中鍵全域快速控制**：支援按一下、長按中鍵快速切換剪貼簿，可自訂觸發方式與長按時間以防止誤觸；不影響系統原生功能，關閉後自動釋放資源，視窗內操作不會誤收視窗。
- **⌨️ 全新快速鍵錄製與推薦**：升級快速鍵設置介面，支持一鍵錄製快速鍵，點擊下拉按鈕可查看常用快速鍵與修飾鍵建議面板。
- **🔒 Win+V 全局無縫接管**：完美相容 Windows 系統剪貼簿，無需修改系統配置，一鍵接管 / 自動恢復原生 Win+V 功能，非侵入式體驗。

##### 💫 體驗優化
- **📌 視窗釘住功能全面優化**：升級視窗固定駐留能力，頂欄、側邊導航兩種版面配置下，重新打開視窗均可保留釘住狀態。同時系統可識別視窗釘住狀態，在 Windows 不奪焦模式下，點擊視窗外部區域不會自動關閉。
- **🔒 Windows 不奪焦靜默貼上模式極致體驗升級**：
  - **新增 `空格鍵` 啟用搜尋**：非啟用狀態下按下空格鍵，能夠快速啟用搜尋框並錄入內容。
  - **新增 `ESC 鍵` 極速隱藏**：不奪焦模式下按下 `ESC` 鍵可一鍵隱藏視窗並退回後台。
  - **精簡 `退格鍵` & `刪除鍵` 攔截**：在非啟用狀態下徹底移除了對退格、刪除鍵的攔截，避免編輯內容時誤刪剪貼簿資料，保障資料安全。
- **📐 擴充側邊導航模式拖拽區域**：側邊導航版面配置中，側邊欄、視窗頂欄、搜尋及標籤區域均可直接拖拽移動。

##### 🐞 bug 修復
- **🧹 修復焦點與通知事件記憶體洩漏問題**：優化元件銷毀時的事件清理邏輯，切換導航模式、重載版面配置後，可正常釋放各類事件監聽，徹底解決監聽堆積、後台運行異常及記憶體洩漏問題。

#### Pro.5.5

##### ✨ 新增功能
- **📐 剪貼簿“側邊導航”版面配置上線**：在「偏好設定 - 一般設定 - 外觀設定 - 檢視模式」中新增側邊導航選項。開啟後，剪貼簿視窗左側將展示窄款側邊欄，縱向排布應用程式 Logo、八大分類圖示、收藏夾與設定按鈕；右側搭配通欄搜尋框，介面版面配置更合理，操作體驗更沉浸高效。
- **🏷️ 收藏夾類型標籤篩選與配置**：側邊導航模式下，點擊收藏按鈕即可在頁面頂部調出分類篩選列。同時新增收藏夾類型標籤顯示開關，支援自主勾選文字、圖片、連結等標籤，按需篩選內容。
- **📋 收藏夾排序**：在「偏好設定 - 剪貼簿 - 排序設定」分組，新增預設開啟的「收藏夾排序」開關，開啟後可透過右鍵選單（置頂、上移、下移、置底）或卡片頂部專屬拖曳手柄調整條目順序。收藏夾排序功能與常規分組的自動排序、時間線置頂規則完全解耦，實現互不干擾的雙軌排序能力。
- **🔍 搜尋支援空格分詞與萬用字元**：重構搜尋檢索邏輯，支援空格分隔多個關鍵字實現交集查詢；同時相容 `*`、`?` 萬用字元語法，有效提升內容檢索的精準度與靈活性。
- **🔒 新增 Windows 不奪焦靜默貼上模式**：在「偏好設定 - 剪貼簿 - 視窗設定」新增該功能開關，開啟後將自動關閉視窗預設聚焦。喚醒剪貼簿時不會搶占當前視窗焦點，可實現檔案重命名、雙擊貼上等無干擾的靜默操作。同時按下左右方向鍵、空格按鍵、`Ctrl+F`、`/` 快捷鍵，或直接點擊搜尋框，視窗會自動啟用並獲取焦點，兼顧靜默貼上與高效檢索的使用需求。

##### 💫 體驗優化
- **🗃️ 優化大數據批量刪除體驗**：重構批量刪除、自動清理的底層查詢邏輯，採用輕量化資料檢索方案，刪除時不再載入文字、圖片等大體積欄位，大幅降低記憶體佔用，徹底解決海量資料清理時介面卡頓、假死的問題。
- **⚡ 搜尋輸入增加防抖處理**：搜尋框配置 `300ms` 防抖延時，避免連續輸入時頻繁發起資料庫查詢，徹底解決打字過程中介面掉幀、卡頓問題。
- **🖼️ 圖片載入全面優化**：圖片元件啟用原生懶載入特性，並新增背景多執行緒非同步預解碼佇列。可視區外圖片提前完成解碼與快取，滾動頁面時消除白塊閃爍，實現流暢順滑的瀏覽體驗。

##### 🏗️ 架構優化
- **🧹 新增資料庫智慧整理機制**：資料刪除後自動執行查詢計畫優化，提升 SQLite 資料庫運行效率；當單次批量刪除或自動清理條目數量≥50 條時，自動觸發資料庫收縮，及時釋放閒置磁碟空間。
- **🔒 增加獨占式維護鎖**：透過單例鎖機制串行執行資料庫優化、收縮等維護操作，有效避免高並發刪除場景下出現資料庫鎖表、寫入衝突，保障後台任務穩定運行。
- **🗄️ 資料庫索引深度優化**：為剪貼簿資料表的高頻篩選、排序欄位新建多組索引，規避全表掃描與排序開銷，大幅提升大資料量下清單載入、分類切換的檢索響應速度。

#### Pro.5.4

##### ✨ 新增功能
- **🔄 輕量資料同步模組落地**：偏好設定加入「資料同步」入口，分為「基礎配置 / 推播資料 / 接收資料」三大模組。
- **📤 主動推播能力接入主介面**：歷史紀錄頭部操作按鈕、右鍵選單、偏好設定操作按鈕均新增「推播」，支援把單條剪貼簿內容主動發到外部設備。
- **⚙️ 自動推播策略**：支援「關閉 / 僅收藏 / 自訂標籤」三種模式；其中僅收藏模式會在條目標星時自動推播，自訂模式支援按內容類型標籤篩選。
- **📡 自動接收服務**：新增本地 HTTP 接收介面，支援 text / html / rtf / files / image 寫入本地歷史，並可選自動回寫系統剪貼簿。
- **🖼️ 圖片中轉策略**：圖片推播支援「區域網臨時直鏈 / Webhook 網關 / WebDAV / 本地目錄映射 / 直接攔截」五種策略。
- **📍 遠端來源標記**：遠端寫入內容會保留來源設備資訊，並在清單中顯示遠端設備圖示。

##### 💫 體驗優化
- **💻 程式碼預覽換行優化**：程式碼高亮預覽改為真實高度裁切並支援自動換行，長程式碼不再橫向溢出。
- **🎨 HTML 可讀性增強**：新增主題智慧感知與文字對比度自動修正能力，解決明暗主題下文字顯示不清的問題。
- **♻️ 防同步迴圈**：新增傳輸回寫指紋保護，避免「接收後自動複製」再次觸發本地監聽，造成重複入庫或重複推播。
- **🖼️ 圖片接收穩定化**：接收圖片統一轉存為 PNG，並補充尺寸/體積元資料；針對 SVG / HEIC / HEIF 格式圖片需要在上傳時統一轉換為普通圖片。

##### 🏗️ 架構優化
- **🧵 推播鏈路佇列化**：推播改為非同步佇列處理，避免阻塞主剪貼簿監聽流程。
- **🔐 配置分層儲存**：敏感憑證走系統憑證儲存，普通配置資訊落入使用者設定檔內。

##### 🐞 bug 修復
- **🧲 操作按鈕配置重構**：修復了「自訂操作按鈕彈窗」中無法拖曳調整按鈕順序的問題，重構底層拖曳邏輯以相容並存的「拖曳匯入備份檔案」功能。

#### Pro.5.3

##### ✨ 新增功能
- **🚀 貼上為純文字升級全域快捷鍵**：重構「貼上為純文字」快捷鍵能力，打破僅限界內使用的侷限，正式升級為全域快捷鍵。軟體後台時一鍵貼上剪貼簿最新內容，前台時貼上選取內容，使用更靈活。

##### 💫 體驗優化
- **📊 重構剪貼簿類型識別引擎**：修復了因開啟「複製為純文字」模式後，從辦公軟體（Excel/WPS）複製資料被誤存為圖片的問題。
- **🎯 複合型表格資料智慧探測**：針對從辦公軟體（Excel/WPS）複製資料與瀏覽器複製圖片時極其相似的內容特徵，引入了四維探測體系，實現了對辦公表格資料的絕對精準定位與純淨化提取。
- **🧹 自動清理表格暫存檔案**：識別表格後，自動刪除無用暫存圖片，避免磁碟產生殘留檔案。
- **🔘 優化預設快捷按鈕**：預設開啟 9 項常用快捷操作並調整排序（偏好設定 → 剪貼簿 → 顯示設定 → 操作按鈕），全方位提升新使用者開箱即用的便捷體驗。
- **🖱️ 劃詞貼上功能優化**：將原「文字片段選取」功能更名為「劃詞貼上」（偏好設定 → 剪貼簿 → 內容設定 → 劃詞貼上），功能意圖表達更為直觀；同時在偏好設定中將其位置移動至「自動貼上」下方，設定時更直觀。
- **📊 儲存統計視圖空間優化**：深度精簡儲存統計頁面冗餘的包裹留白，左側統計摘要區字號與寬度動態縮窄，為橫向統計圖表提供最大化視覺化空間。

##### 🐞 bug 修復
- **⌨️ 修復貼上為純文字快捷鍵失效問題**：深度重構底層系統級按鍵注入邏輯，解決「貼上為純文字」和「快速貼上」時按鍵異常導致的無響應問題，貼上操作更穩定。
- **📈 修復統計圖表邊緣溢出**：徹底修復在極窄視窗尺寸下，統計圖表右側內容被強行擠出視窗外的佈局溢出問題。
- **⏰ 儲存統計時間選擇器自適應優化**：修復了在選擇「自訂」時間範圍時，右側超長的日期區間選擇框被強行擠出視窗外的佈局溢出問題。

#### Pro.5.2

##### ✨ 新增功能
- **📦 本地資料篩選匯出**：全新重構本地資料匯出引擎，新增「範圍 (全部 / 僅收藏) + 12 種內容類型」的多維組合篩選匯出能力。匯出的備份檔案僅包含匹配的內容及關聯圖片，且能直接兼容匯入還原，備份更精準、更實用。
- **🏷️ 全局標準化備份命名體系**：統一了 WebDAV 和本地匯出的備份檔案命名規則（`應用程式名.時間戳.設備名.系統.模式.副檔名`），並劃分出`full`（完整）、`lite`（輕量）、`filter`（篩選）、`favs`（僅收藏） 4 種備份模式，備份檔案管理更清晰。
- **📥 拖曳匯入備份檔案**：新增拖曳本地備份檔案還原資料的功能，匯入成功後自動重整剪貼簿清單，無須手動重啟應用程式，還原資料更便捷。
- **⌨️ 雙擊修飾鍵快捷喚起**：打破傳統組合鍵限制，新增「雙擊修飾鍵（`Ctrl` / `Alt` / `Shift`）」快捷打開剪貼簿視窗，設定面板新增「錄製 / 雙擊」切換開關，明確隔離`空間組合鍵`與`時間雙擊鍵`，配置更直觀、操作更順手。

##### 💫 體驗優化
- **✂️ 智能剔除邊緣空白**：複製信箱、連結、路徑或顏色值時，自動去掉內容首尾不小心複製的多餘空白，避免內容被誤判為`純文字`，保證類型識別精準。
- **🎨 程式碼語法高亮重構**：升級程式碼高亮效果，採用和 VS Code 一致的專業配色方案，程式碼可讀性大幅提升；同時優化視覺效果，程式碼區塊與內容卡片融合更自然，閱讀更沉浸。
- **🔄 匯入還原無感重載**：重構`匯入備份檔案`與`從 WebDAV 還原`的資料加載機制，實現匯入後無須重啟應用程式，剪貼簿歷史清單即時重整的無感熱重載體驗。
- **🛡️ 精準識別剪貼簿真實變化**：優化後台監聽邏輯，能精準過濾第三方軟體或系統導致的`假複製`事件，只響應真實的剪貼簿內容變更，後台運行更精準、更高效。
- **🔍 優化相同內容複製的來源識別**：連續在不同應用程式複製相同的文字 / 圖片時，能精準更新內容的來源應用程式圖示和名稱，且內容會正常置頂排序，符合使用直覺。
- **🔒 保護歷史紀錄來源準確性**：修復`自動排序`模式下，貼上操作會錯誤修改歷史紀錄來源應用程式的問題，確保紀錄的來源資訊準確無誤。
- **📊 清晰顯示表格框線**：從 Excel 等應用程式複製表格內容時，新增了安全、原生且不衝突的底層網格線保底樣式，極大增強了資料呈現的結構性和可讀性。
- **✂️ 優化 HTML 內容顯示效果**：修復複製 HTML 內容時，清單卡片首尾出現大片空白的問題，自動修剪多餘空白，顯示更緊湊美觀。

##### 🐞 bug 修復
- **📧 信箱分組顯示空白修復**：修復點擊`信箱`分組後，實際已正確分類的信箱紀錄無法顯示的問題。
- **📌 置頂視窗貼上後異常關閉修復**：徹底修復置頂狀態的剪貼簿視窗，在執行各類貼上操作後被強制關閉的問題，確保置頂視窗穩固顯示。

#### M05.1

##### ✨ 新增功能（全新儲存空間管家體驗）
- **📊 全景圖表視界**
  - **📋 多維儲存統計面板**：新增 12 大核心內容類型儲存統計，直觀展示空間佔用情況；支援「全部/收藏」獨立切換，可單獨查看收藏記錄佔用資料。
  - **⚡ 超高效能統計引擎**：重構物理體積溯源與大盤統計算法，移除阻塞性磁碟 IO 遍歷，統一改為記憶體級資料庫查詢，上萬條記錄、數百 GB 資料也能瞬時載入。
  - **📈 動態視覺平衡演算法**：針對大小懸殊的「長尾資料」，採用開方平滑映射公式，兼顧直觀對比的同時，避免小體積資料視覺「隱身」。
  - **🎯 全景資料透傳互動**：優化柱狀圖互動區域，滑鼠懸停即可彈出懸浮卡片，即時顯示對應類型的條目數量與精確體積。
- **⏳ 靈活的時間線流轉**
  - **⏰ 全域時間智慧過濾**：內建時間篩選器，支援`今天`、`昨天`、`近三天`、`本週`、`本月`及`自訂`日期範圍，快速透視指定時段儲存資料。
  - **🔗 全模組聯動響應**：時間篩選與統計圖表、詳情列表無縫同步，切換時間即可直觀查看資料堆積與變化趨勢。
- **🧹 防誤刪的智慧大掃除**
  - **🗑️ 按類型精準清理**：支援按內容類型多選，一鍵批量清理 `未收藏` / `已收藏` 的冗餘歷史資料。
  - **🖼️ 圖片深度聯動清理**：清理圖片時，系統除了抹除資料庫指紋，同時支援「同步刪除本地圖片檔案」，真正釋放磁碟空間，完美適配自訂儲存路徑。

##### 🐞 Bug 修復
- **🌐 網頁連結識別修復**：優化剪貼簿類型判斷邏輯，純文字格式的URL/郵箱會強制識別為純文字，解決從網頁複製帶樣式連結被誤判為HTML的問題。
- **🎨 顏色代碼誤判修復**：重構顏色識別校驗引擎，僅標準色值，避免普通英文單詞（如 `Tan`、`azure` 等）被誤判高亮。

##### 🐛 原版缺陷修復
- **📂 檔案圖示路徑隔離修復**：深度對接 tauri-plugin-fs-pro 底層 API，檔案圖示將精準儲存至使用者自訂目錄，不再殘留到系統預設 `AppData` 路徑。

#### M05.0

#### ✨ 新增功能
- **📅 多維組合篩選**：新增「日期 + 內容類型」多維度複合篩選能力，點擊頂部漏斗圖示即可喚起篩選面板。支援按日、按月、自訂時間範圍三種獨立日期篩選模式，同時支援 12 種內容類型標籤多選，打造高效精準的內容檢視鏈路。
- **🧩 代碼 (SVG) 智慧識別**：新增代碼層面的 SVG 格式化內容智慧識別，依託「正則初篩元件骨架 + DOMParser 解析 XML 結構」雙重校驗構建安全壁壘，並將 SVG 識別結果歸類展示為明確的 `代碼 (SVG)` 標籤。
- **📌 詳情級全域 Tooltip**：滑鼠懸停在內容條目頭部任意文字區域，即可詳盡追溯內容全量核心屬性，涵蓋來源、內容類型、字元數（檔案大小 / 解析度）、建立時間等資訊，詳情查看更便捷。

#### 💫 體驗優化
- **📝 精準時間自適應顯示**：重構剪貼簿歷史記錄時間顯示邏輯，當相對時間（如「3 天前」）跨度超過 3 天時，將自動切換為 `YY/M/D H:mm` 格式的絕對時間，時間追溯更清晰、更具參考性。
- **🧹 頂部控制元件佈局重組**：優化搜尋懸浮窗頂部佈局，合理收窄搜尋輸入框視覺寬度，同時將「置頂 / 取消置頂」「偏好設定」功能按鈕組前置整合，大幅提升高頻操作的觸達效率。
- **🏗️ 焦點管理全鏈路回退**：「不奪焦靜默視窗」「視窗跟隨游標位置」底層實現全量回退至原版 v0.6.0-beta.3 版本，剝離強依賴 Win32 API 的後臺輪詢執行緒與額外掛載介面，恢復主視窗原生焦點捕獲機制。

#### 🐞 bug 修復
- **🚀 啟動時分組切換失效**：徹底解決「偏好設定 - 剪貼簿 - 視窗呼出時切換至全部分組」關鍵配置失效的問題，確保應用啟動時嚴格遵循使用者設定的分組規則。
- **📰 複合型代碼誤判**：徹底修復從 IDE 開發環境或網頁中複製同時攜帶「純文字代碼 + HTML 富文字語義」混合片段時，內容被粗暴定性進而被強制識別為 `HTML` 超文字類型的底層優先級越權漏洞，確保複製的代碼內容可被精準識別歸類。
- **🌐 HTML 代碼誤判**：重構 HTML 代碼底層判定邏輯，補全對聲明頭部、大小寫不敏感場景的正則相容支援，徹底解決因骨架校驗失效導致的內容誤判問題，識別準確率大幅提升。
- **📝 Markdown 誤判**：徹底重構 Markdown 檢測邏輯，升級為「核心語法正向加權 + 非法特徵負向懲罰 + 嚴格前綴邊界校驗」的複合型智慧判定引擎，有效解決 JS 等其他代碼片段被誤判為 Markdown 格式的問題。
- **📋 JavaScript 代碼誤判**：重構底層代碼語言檢測引擎，將 JavaScript 判定優先級調至最高，針對性補充前端建構編譯產物的強特徵詞；同時移除 C++ 檢測規則中易與 JS 位移運算衝突的通用運算子（`<<`、`>>`），新增 JS 特徵詞「反向查殺」校驗機制，徹底解決混淆、壓縮後的前端 JS 生產代碼被誤識別為「代碼 (C++)」的頑固問題。
- **🗑️ 歷史遺留垃圾清剿**：深度比對甄別並徹底清理廢棄掛載的殘存臨時檔案，從根源杜絕冗餘資料對後續功能重構的干擾。

#### 🐛 原版缺陷修復
- **👁️ 條目頭部全寬釋放**：取消了原版介面中為右側隱形操作按鈕強行預留的固定寬度「自留地霸佔」行為，未選中狀態下的記錄頭部屬性列可實現 100% 全寬顯示，內容展示空間更充足、資訊呈現更完整。

### M04.x <font size="-2" color="gray">（基於原版v0.6.0-beta.3分支）</font>

#### M04.6

##### 🐞 bug 修復
- **🔀 分組切換內容顯示異常**：修復了剪貼簿視窗快速連續切換分組時內容顯示不正確的競態條件問題，根源是元件加載狀態下第二次分組點擊請求被丟棄所導致。

#### M04.5

##### 🏗️ 架構優化
- **🛡️ 向下相容加固**：遵循「發送保守、接收寬容」的魯棒性原則，解決版本降級時因資料結構不相容導致的崩潰問題。
- **🗄️ 資料庫查詢防護**：重構資料庫查詢層，採用「統一列定義 → 按需查詢」架構，由 `historyColumnDefs` 單一資料來源同時驅動建表與查詢。低版本會自動忽略高版本資料庫新增的未知列，徹底杜絕因結構差異導致的降級崩潰。
- **⚙️ 設定檔處理防護**：新增 `strictDeepAssign` 嚴格深度合併機制，配置同步時僅接受當前版本已定義的配置項，高版本寫入的未知配置欄位將被靜默丟棄，防止狀態污染引發的渲染異常。

##### 💫 體驗優化
- **🎨 文字選取高亮優化**：將剪貼簿內文字片段選取的高亮背景色改為藍色 `#4096FF`，文字顏色設為白色 `#FFFFFF`。
- **📍 返回頂部按鈕優化**：統一調整剪貼簿視窗右下角「返回頂部」按鈕位置，底部與右側邊距保持對等。

##### 🐞 bug 修復
- **📋 來源應用資訊修復**：修復剪貼簿 `autoSort` 開啟時，最新條目來源圖示 / 名稱錯誤顯示為當前活動視窗資訊的問題，改為保留原始複製時的應用資訊。
- **💥 降級安裝崩潰修復**：修復從高版本降級安裝低版本時，剪貼簿視窗因資料庫多餘欄位導致 `Cannot destructure property 'key'` 渲染崩潰的問題（僅對當前及後續版本生效）。

#### M04.4

##### ✨ 新增功能
- **✂️ 支援文字片段選取**：可在剪貼簿視窗中選取指定文字片段，透過自動彈出的浮動工具列或增強右鍵選單快速執行「複製 / 貼上」操作，支援`純文字`、`富文字`、`HTML`類型。
- **🔛 文字片段選取獨立開關**：可按需啟用或禁用部分文字選取功能（偏好設定 → 內容設定 → 文字片段選取）。

#### M04.3

##### ✨ 新增功能
- **🎨 CMYK顏色識別**：新增 CMYK 顏色格式的智慧提取與預覽支援。

##### 💫 體驗優化
- **🖱️ 右鍵選單重構**：
  - **功能分組**：按「複製貼上類 / 操作類 / 編輯類」細分歸類，關聯功能聚合排布，高頻連續操作就近放置，縮短操作距離，提升選擇效率。
  - **功能排布**：依據操作流程先後與使用頻率優化功能排序。
  - **專屬定製**：針對`純文字`、`富文字`、`HTML`、`圖片`、`檔案`、`Markdown`、`連結`、`路徑`、`代碼`、`郵箱`、`顏色`、`指令` 12 種剪貼板內容類型，分別量身定製專屬選單項。

##### 🐞 bug 修復
- **⚙️ 操作按鈕配置修復**：補齊偏好設定中操作按鈕缺失的「執行」選項，支援在設定介面正常配置顯隱和順序。
- **🎨 顏色分組同步修復**：修復在顏色專屬分組內複製新顏色內容時，列表未動態置頂更新的顯示同步問題。

#### M04.2

##### ✨ 新增功能
- **🔍 Windows 路徑與指令智慧識別**：自動識別環境變數路徑 (`%APPDATA%` 等)、Shell 資料夾 (`shell:startup` 等)、檔案系統路徑 (`C:\Windows` 等) 和管理工具指令 (`regedit` 等)，支援右鍵選單和操作按鈕一鍵快速開啟或執行。
- **🔄 設定檔自動同步**：偏好設定變更時自動同步設定到使用者自訂目錄，確保備份設定始終為最新。

#### M04.1

##### ✨ 新增功能
- **☁️ WebDAV 雲備份體系完善**：
  - **精簡與完整備份解耦**：細分“完整”與“精簡”兩種備份規則，支援手動、自動備份場景下的獨立管控，靈活適配不同備份需求。
  - **自動排程引擎**：內建獨立的前端任務排程引擎，實現了靈活的“定時”、“間隔”以及“Cron運算式”三種迴圈模式，實現完整備份與精簡備份雙線並行排程，提升備份效率。
- **📝 新增 Markdown 支援**：採用全新計分權重正則檢測法，精準判斷Markdown格式，徹底規避普通文字、代碼被誤判歸類的問題，同時支援 Markdown 格式化沉浸編輯，優化編輯體驗。
- **🖼️ 圖片定位目錄**：圖片類型支援一鍵拉起檔案總管精準定位原檔案所在目錄。

##### 💫 體驗優化
- **☁️ 無縫恢復互動**：重構WebDAV恢復彈窗喚醒機制，會優先即刻渲染骨架及 Loading UI，後臺採用非阻塞式拉取雲端記錄，徹底解決網路延遲導致的介面假死、無反饋頓挫問題，提升操作流暢度。
- **💾 備份壓縮包相容性優化**：運用獨立臨時隔離區 (Staging Dir) 技術，使得 WebDAV 備份結構與原生本地“匯出資料”完全一致，實現雙入口無縫相容，備份與恢復全程無阻礙。

##### 🐞 bug 修復
- **🌐 WebDAV 跨端阻斷**：利用 Rust 原生鉤子 (`ensure_remote_dir`) 自動在目標雲端探測並遞迴構建父級目錄，徹底解決了因上級目錄遺失而拋出的 405 Method Not Allowed 錯誤。
- **💾 資料庫備份異常截斷**：防禦性修復了因底層 Kysely 歷史表中存在 NULL 系列空白欄位導致的 Invalid column type Null 報錯中止問題，保障資料庫備份的完整性與穩定性。
- **📋 類型判斷權重優化**：徹底修復從 Excel 等表格軟體複製儲存格時，由於同時持有 Image + HTML 格式組合，導致文字內容被強制降權、誤識別為圖片的底層漏洞，確保複製內容識別準確。

### M03.x <font size="-2" color="gray">（基於原版v0.6.0-beta.3分支）</font>

#### ✨ 新增功能
- **☁️ WebDAV 雲端備份**：支援透過 WebDAV 協議將剪貼板資料備份到雲端（堅果雲、NextCloud 等），提供手動備份、自動定時備份、備份數量限制、一鍵恢復等完整功能。
- **🔒 本地憑證安全儲存**：WebDAV 的伺服器地址、使用者名稱、密碼與路徑等敏感設定資訊均採用原生呼叫系統級 Windows 憑證管理員（Credential Manager）進行高強度加密安全持久化儲存，應用層面不再以明文暴露，徹底杜絕隱私洩漏風險。
- **🗑️ 可選刪除本地圖片**：刪除圖片內容時，二次確認彈窗增加"刪除本地檔案"選項（預設選中），可選擇保留本地檔案僅刪除剪貼板記錄。

### M02.x <font size="-2" color="gray">（基於原版v0.6.0-beta.3分支）</font>

#### ✨ 新增功能
- **🎨 獨立群組與顏色直出**：新增 "連結"、"顏色"、"代碼"、"郵箱" 原生專屬群組分類。精準提取並高亮顯示 RGB/RGBA 等顏色格式，路徑連結高亮直擊目標。
- **📝 豐富二次沉浸編輯**：支援文字等富內容資料的獨立二級彈窗編輯與系統級別快速檔案定位。
- **💻 高亮代碼語法預覽**：自動偵測複製的代碼片段並賦予媲美 IDE 介面的原生級代碼語法高亮渲染（偏好設定 → 剪貼板 → 顯示設定 → 代碼語法高亮）。
- **🎯 資料來源截取**：提供來源拷貝 App 的 Icon 與應用名稱獲取（偏好設定 → 剪貼板 → 顯示設定 → 記錄應用來源）。
- **⚡️ 原生快速存取**：支援在檔案總管中直接開啟檔案路徑，在瀏覽器中一鍵開啟網頁連結，以及呼叫系統預設圖片查看器查看圖片內容。
- **🔢 代碼/檔案顯示行數自訂**：擴展了行數適配能力，支援代碼和檔案類型（偏好設定 → 剪貼板 → 顯示設定 → 代碼/檔案顯示行數）。

> **致謝**：本專案中“獨立群組與顏色直出”、“豐富二次沉浸編輯”、“高亮代碼語法預覽”、“🎯 資料來源截取”與“原生快速存取”的實現思路參考自 [EcoPaste-Sync](https://github.com/Ruszero01/EcoPaste-Sync)，特此致謝。

#### 🐛 原版缺陷修復
- **📸 截圖完美轉儲**：重構 SQLite 日志行為及底層 FS 映射鏈路，讓截圖圖片完美儲存至指定的自訂本地目錄，徹底解決原版截圖因內建庫限制導致自訂目錄崩潰和展示圖片白板紅叉的問題，杜絕 C 槽資源浪費。
- **🔗 複製連結雙重記錄**：徹底修復複製連結時剪貼板會出現兩條完全相同記錄的頑固問題。
- **📊 複製文件表格內容雙重記錄**：徹底修復從 Word、Excel 等文件的表格中複製內容時，剪貼板會同時出現兩條完全相同重複記錄的問題。

### M01.x <font size="-2" color="gray">（基於原版v0.6.0-beta.3分支）</font>

#### 🔄 動態展開收起與沉浸體驗
- **展開完整資料**：當內容超過顯示限制時提供展開/收起按鈕，圖文狀態不受虛擬滾動破壞，滾動後狀態始終保持。
- **不奪焦靜默視窗（Windows）**：呼出視窗時原應用保持主焦點，雙擊條目直接靜默粘貼；點擊視窗外部立即自動隱藏。
- **自動跟隨輸入游標**：視窗跟隨編輯器輸入游標位置貼圖顯示，實現心流體驗。
- **偏好設定架構重製**：新增「顯示設定」分組，精細化分類各項高級設定。

#### 📏 文字與圖片顯示高級適配
- **文字顯示行數自訂**：偏好設定 → 剪貼板 → 顯示設定 → 文字顯示行數（1 - 50 行）
- **圖片預渲染高度縮放**：支援靈活調整圖片高度，超過後智能展開或統一收起（50 - 500 像素）

#### ⚙️ 配置持久化
- 所有新增配置項自動儲存到用戶資料目錄，應用更新後配置不會丟失，無需手動匯出/匯入。

#### 🐛 原版缺陷修復
- **📋 剪貼板類型誤判**：徹底修復網路圖片可能會因 html 權重被錯誤識別為網頁的問題。深度改寫判定邏輯，賦予圖片層級最高優先順序。
- **💾 備份通道恢復**：原版中由於許可權限制被遺棄的資料備份入口，全面解封並確保匯入匯出穩定運行。

#### 🔄 自動同步上游
- 每天自動檢查上游 3899/EcoPaste-Pro 的更新
- 有新版本時跨分支自動合併並觸發增量構建
- 如遇重構衝突會創建 Issue 提供手工接軌通道

---

## 問題迴響

1. 🔍 優先查閱[常見問題](https://ecopaste.cn/problem/macos/damage)或瀏覽已有 [issues](https://github.com/3899/EcoPaste-Pro/issues)。

2. ❓ 如果問題仍未解決，請提交新的 [issue](https://github.com/3899/EcoPaste-Pro/issues/new/choose)，並附上詳細描述，方便我們快速定位和解決。


