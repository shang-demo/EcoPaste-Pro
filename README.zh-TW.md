...
- **🔒 新增 Windows 不奪焦靜默貼上模式**：在「偏好設定 - 剪貼簿 - 視窗設定」新增該功能開關，開啟後將自動關閉視窗預設聚焦。喚醒剪貼簿時不會搶占當前視窗焦點，可實現檔案重命名、雙擊貼上等無干擾的靜默操作。同時按下左右方向鍵、`Ctrl+F`、`/` 快捷鍵，或直接點擊搜尋框，視窗會自動啟用並獲取焦點，兼顧靜默貼上與高效檢索的使用需求。
