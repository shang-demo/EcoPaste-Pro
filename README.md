...
- **🔒 新增 Windows 不夺焦静默粘贴模式**：在「偏好设置 - 剪贴板 - 窗口设置」新增该功能开关，开启后将自动关闭窗口默认聚焦。唤醒剪贴板时不会抢占当前窗口焦点，可实现文件重命名、双击粘贴等无干扰的静默操作。同时按下左右方向键、`Ctrl+F`、`/` 快捷键，或直接点击搜索框，窗口会自动激活并获取焦点，兼顾静默粘贴与高效检索的使用需求。
